  ...
  useEffect(() => {
    ...
    async function loadAudioSource() {
      try {
        ...
        if (track?.previewUrl) {
          try {
            const fetchUrl = buildFetchUrl(track.previewUrl);
            ...
            console.warn(
              'Fetch proxy fallito su URL iTunes preview, tento caricamento diretto:',
              corsErr
            );
          }
          ...
          try {
            if (!isCancelled && wavesurferRef.current) {
              await wavesurferRef.current.load(track.previewUrl);
              return;
            }
          } catch (directErr) {
            console.warn('Wavesurfer direct load fallito, uso synth fallback:', directErr);
          }
        }
    ...
    try {
      ...
      const fadeTag =
        fadeInSec > 0 || fadeOutSec > 0
          ...
          : '';
      const filename = `${safeArtistName}_-_${safeTrackName}_[Ringtone_${Math.round(
        startTime
      )}-${Math.round(endTime)}s${fadeTag}].mp3`;

      downloadBlobAsFile(blob, filename);
    } catch (err) {
      alert("Error exporting MP3 file: " + err);
    } finally {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2.5 border-b border-white/10">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-slate-100 font-bold text-xs sm:text-sm truncate drop-shadow-sm flex items-center gap-1.5">
              <span>Advanced Audio Studio</span>
            ...
            <p className="text-[10px] sm:text-[11px] text-slate-300 truncate">
              {studioMode === 'single'
                ? 'Quick ringtone trimming with Fade In/Out'
                : 'Multi-segment arrangement & seamless crossfade'}
            </p>
          </div>
        </div>

        {/* Mode Selector Tabs (Single Trim vs Multi-Segment Studio) */}
        <div className="flex items-center gap-1.5 bg-slate-950/90 p-1 rounded-xl border border-white/10 shrink-0">
          ...
          >
            ...
            <span>Multi-Segment Studio</span>
          </button>
        </div>
      </div>
      ...
      <div className={studioMode === 'single' ? 'flex flex-col gap-3 sm:gap-4' : 'hidden'}>
        {/* PLAYBACK CONTROLS, LOOP & PRESETS */}
        <div className="bg-slate-950/60 border border-white/10 rounded-xl p-2.5 sm:p-3 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-1.5 text-[11px]">
            ...
            <button
              ...
              type="button"
              onClick={handleReset}
              title="Reset selection to full track duration (0s - end)"
            ...
            {!isReady && (
              <div className="absolute inset-0 flex items-center justify-center bg-slate-950/70 z-20 backdrop-blur-xs">
                <div className="flex items-center gap-2 text-indigo-300 text-xs font-semibold">
                  ...
                  <span>Generating audio waveform in real-time...</span>
                </div>
              </div>
            )}
          ...
          <div className="bg-slate-950/70 border border-white/10 rounded-xl p-2.5 sm:p-3 flex flex-col gap-2.5 shadow-inner">
            <div className="flex items-center justify-between gap-2 text-xs">
              <div className="flex items-center gap-1.5 font-semibold text-slate-200">
                ...
                <span>Ringtone Range (Start & End)</span>
              </div>
          ...
          <div className="bg-slate-950/70 border border-white/10 rounded-xl p-2.5 sm:p-3 flex flex-col gap-2.5 shadow-inner">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5 text-slate-200 font-semibold text-xs">
                ...
                <span>Audio Fades (Fade In / Fade Out)</span>
              </div>
              <div className="flex items-center gap-2">
                {(fadeInSec > 0 || fadeOutSec > 0) ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
          ...
          >
            {isSaved ? (
              <>
                ...
                <span>Saved to My Ringtones!</span>
              </>
            ) : (
              <>
          ...
          >
            ...
            <span>{isExporting ? 'Generating MP3...' : 'Download MP3 File'}</span>
          </button>
        </div>
      </div>
    </div>
