...
export const SongCard: React.FC<SongCardProps> = ({
  ...
  const togglePreview = async (e: React.MouseEvent) => {
    ...
    const { startTime = 0, endTime = 30, fadeInSec = 0, fadeOutSec = 0 } = trimRangeRef.current;
    ...
    const audioUrl = track.previewUrl || (track.fileBlob ? URL.createObjectURL(track.fileBlob) : '');
    ...
    if (audioUrl) {
      ...
      const setupInterval = () => {
        ...
        timerRef.current = window.setInterval(() => {
          ...
          if (ct >= endTime || (previewAudioRef.current.duration && ct >= previewAudioRef.current.duration)) {
            stopPreview();
          } else {
            try {
  ...
  return (
    <div
      ...
      className="relative overflow-hidden bg-slate-950/80 border border-slate-700/80 rounded-2xl p-3.5 sm:p-4.5 shadow-2xl flex flex-col gap-3 hover:border-slate-500/80 transition-all group"
    >
      {/* 1. COPERTINA ALBUM INGRANDITA A TUTTA LUNGHEZZA SUL LATO SINISTRO CON SFUMATURA VERSO DESTRA */}
      <div className="absolute left-0 top-0 bottom-0 w-48 sm:w-80 md:w-[420px] lg:w-[520px] pointer-events-none overflow-hidden rounded-l-2xl z-0">
        <img
          src={artwork}
          alt={`${track.trackName} cover art`}
          className="w-full h-full object-cover object-left sm:object-center filter contrast-110 brightness-105 opacity-90 sm:opacity-95 transition-transform duration-700 group-hover:scale-105"
          referrerPolicy="no-referrer"
        />
        {/* Sfumatura morbida e progressiva verso destra (dal vivo della copertina al nero sfumato della card) */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-slate-950/25 via-45% to-slate-950" />
        <div className="absolute inset-0 bg-gradient-to-b from-slate-950/25 via-transparent to-slate-950/60" />
      </div>

      {/* 2. SFONDO ATMOSFERICO GENERALE DELLA CARD */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-2xl z-0">
        <img
          src={artwork}
          alt=""
          aria-hidden="true"
          className="w-full h-full object-cover object-center opacity-20 scale-105 filter blur-[2px] brightness-90"
          loading="lazy"
          referrerPolicy="no-referrer"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-slate-950/80 via-slate-950/90 to-slate-950/95" />
      </div>

      {/* CONTENT LAYER */}
      <div className="relative z-10 flex flex-col gap-3">
        {/* SONG CARD HEADER: 3 Structured Lines + Play/Pause & Taglia Suoneria */}
        <div className="relative p-3 sm:p-4 rounded-2xl bg-gradient-to-r from-slate-950/95 via-slate-950/85 to-slate-950/60 border border-white/10 backdrop-blur-md shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
          {/* Header Left: Prominent Foreground Album Artwork + Details */}
          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto flex-1 min-w-0">
            {/* Prominent High-res Album Artwork in Foreground */}
            <div className="relative w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden shadow-2xl border-2 border-white/20 shrink-0 group/cover">
              <img
                src={artwork}
                alt={track.trackName}
                ...
                referrerPolicy="no-referrer"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-black/10 group-hover/cover:opacity-80 transition-opacity" />
            </div>
            ...
            <div className="min-w-0 flex-1 flex flex-col gap-1.5 w-full">
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                <button
                  ...
                  title={
                    isPlayingPreview
                      ? 'Pause preview of trimmed segment'
                      : `Listen to trimmed segment preview (${trimRange.startTime.toFixed(1)}s - ${trimRange.endTime.toFixed(1)}s)`
                  }
                ...
                {track.isLocalFile ? (
                  <span className="flex items-center gap-1 bg-pink-950/90 text-pink-300 border border-pink-700/80 px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-bold shrink-0 backdrop-blur-xs shadow-xs">
                    ...
                    <span>Local File {track.fileSizeFormatted ? `(${track.fileSizeFormatted})` : ''}</span>
                  </span>
                ) : (
                ...
                <button
                  type="button"
                  onClick={togglePreview}
                  title={`Listen to trimmed segment preview (${trimRange.startTime}s - ${trimRange.endTime}s)`}
                ...
                >
                  {isPlayingPreview ? (
                    <>
                      ...
                      <span>Playing ({segmentDurationSec}s)</span>
                    </>
                  ) : (
                    <>
                      <Volume2 className="w-3 h-3 text-indigo-400" />
                      <span>Trim Preview</span>
              ...
              {/* 2° LINE: Album, Year and Duration */}
              ...
              {/* 3° LINE: Rhythm Analysis (BPM, Tempo & Energy Level) */}
              <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                ...
                {/* Energy Level */}
                <div
                  title={`Energy Level: ${rhythm.energyLevel} (${energyPct}%)`}
          ...
          <div className="flex items-center self-end sm:self-center shrink-0 w-full sm:w-auto justify-end pt-1.5 sm:pt-0 border-t sm:border-t-0 border-slate-800/80">
            <button
              ...
              title={isExpanded ? 'Collapse Studio' : 'Open Ringtone Studio'}
            ...
            >
              ...
              <span>{isExpanded ? 'Collapse Studio' : 'Open Studio'}</span>
              {isExpanded ? (
